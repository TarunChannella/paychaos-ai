/**
 * Phase 3D-E — `POST /api/chaos/runs/{runId}/start-c11-a`.
 *
 * An untrusted execution boundary for starting C11-A's pure failed-payment
 * observation, mirroring `app/api/chaos/runs/[runId]/arm-c07/route.ts`'s
 * exact protections. The ONLY execution input is the `runId` path segment —
 * this route never reads a request body, and never accepts
 * `authorized`/`target`/`url`/`host`/`endpoint`/`fault`/`scenarioId`/
 * `mechanism` from the caller. All actual eligibility/security/execution
 * logic lives in `lib/chaos/c11-execution-service.ts`'s
 * `startC11AFailureObservation` — this route only validates the path shape,
 * enforces the untrusted-caller boundary (session authorization, same-origin
 * — PRE-SEC-010), and maps the service's typed result to a safe HTTP
 * response.
 *
...
import { startC11AFailureObservation } from "@/lib/chaos/c11-execution-service";
...
const SAFE_ERROR_BODY = {
  error: "Chaos start request could not be processed.",
} as const;
...
export async function POST(
  ...
  if (isKnownCrossOriginRequest(request)) {
    logEvent("chaos_c11a_start_request", {
  ...
  try {
    const result = await startC11AFailureObservation(runId);

    if (result.kind === "OBSERVING") {
      logEvent("chaos_c11a_start_request", {
        outcome: "OBSERVING",
        chaos_run_id: result.chaosRunId,
      });
      return NextResponse.json(
        { chaosRunId: result.chaosRunId, status: "RUNNING" },
        { status: 200 },
      );
    }
