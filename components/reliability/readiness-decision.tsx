 ...
 *
 * THE SCORE IS DRAWN, NOT RECOMPUTED. The ring's arc and tone are derived
 * from the verdict and number this component is HANDED. It applies no
 * threshold of its own, so the picture cannot drift from the deterministic
 * RELIABILITY-V1 / GO-LIVE-READINESS-V1 arithmetic behind it.
 */

/** Verdict → ring tone. A presentation mapping, never a judgement. */
const RING_TONE = {
  READY: "pass",
...
export function ReadinessDecision({
  ...
  return (
    ...
    >
      {/* Decision first, score beside it. They stack on narrow screens so
          neither is squeezed, and the ring keeps its own centred alignment
          while the verdict and its reasoning stay left-aligned. */}
      <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
        ...
        >
          <ScoreRing
            score={score}
            tone={RING_TONE[status as keyof typeof RING_TONE] ?? "neutral"}
            size={148}
          />
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 border-t border-border pt-4">
        <Link
          href={href}
          className="inline-flex items-center justify-center rounded-[10px] bg-primary px-4 py-2 text-sm font-medium text-primary-foreground transition-colors hover:bg-[var(--primary-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
