...
describe("066-phase3h-read-models.integration.test.ts — provenance guard", () => {
  ...
  it("14: the integration sequence is pinned — 067 exists and 068 does not", () => {
    ...
    // and is pinned by exact name for the same reason 066 and 067 are.
    ...
    // file, and is pinned by exact name for the same reason 066 through 068 are.
    expect(integrationFiles.filter((name) => name.startsWith("069-"))).toEqual([
      "069-phase4c-root-cause-persistence.integration.test.ts",
    ]);
    expect(integrationFiles.filter((name) => name.startsWith("070-"))).toEqual([
      "070-phase4d-recommendation-persistence.integration.test.ts",
    ]);
    expect(integrationFiles.filter((name) => name.startsWith("071-"))).toEqual([
      "071-phase4e-regression-foundation.integration.test.ts",
    ]);
    ...
    // 076 is the Phase 4F-R3 reliability API proof, added after this file.
    // Like 075 it is READ-ONLY and adds no provenance risk of its own.
