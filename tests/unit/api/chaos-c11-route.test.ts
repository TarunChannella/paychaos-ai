...
// Phase 3D-D: `app/api/chaos/runs/[runId]/execute-c11-b/route.ts` exercised
...
// real Supabase. Mirrors `tests/unit/api/chaos-replay-route.test.ts` (the
// C01 replay route test) exactly, proving only the route's own job:
// validate the runId shape, reject a known cross-origin request, enforce
// the access gate, delegate to the trusted service, and map its typed
// result to a safe response.
...
const executeC11RealWebhookReplayMock = vi.fn();
vi.mock("@/lib/chaos/c11-execution-service", () => ({
  executeC11RealWebhookReplay: (...args: unknown[]) =>
    executeC11RealWebhookReplayMock(...args),
}));
...
async function callExecuteC11B(
  runId: string,
  options: { cookie?: string; origin?: string; secFetchSite?: string } = {},
) {
  const { POST } =
    await import("@/app/api/chaos/runs/[runId]/execute-c11-b/route");
  ...
  const request = new NextRequest(
    `http://localhost/api/chaos/runs/${runId}/execute-c11-b`,
    { method: "POST", headers },
  );
...
describe("POST /api/chaos/runs/[runId]/execute-c11-b — input shape", () => {
  it("returns 400 for a malformed runId and never calls the service", async () => {
    getAccessGateEnvMock.mockReturnValue({ mode: "disabled" });
    const response = await callExecuteC11B("not-a-uuid");
    expect(response.status).toBe(400);
    expect(executeC11RealWebhookReplayMock).not.toHaveBeenCalled();
  });
});

describe("POST /api/chaos/runs/[runId]/execute-c11-b — cross-origin rejection", () => {
  it("returns 403 when Sec-Fetch-Site is cross-site", async () => {
    getAccessGateEnvMock.mockReturnValue({ mode: "disabled" });
    const response = await callExecuteC11B(VALID_RUN_ID, {
  ...
  it("proceeds when Origin matches the request's own origin", async () => {
    getAccessGateEnvMock.mockReturnValue({ mode: "disabled" });
    executeC11RealWebhookReplayMock.mockResolvedValue({
      kind: "COMPLETED",
...
describe("POST /api/chaos/runs/[runId]/execute-c11-b — access gate", () => {
  ...
  it("never accepts a caller-supplied 'authorized' claim — only a verified session cookie", async () => {
    ...
    const { NextRequest } = await import("next/server");
    const request = new NextRequest(
      `http://localhost/api/chaos/runs/${VALID_RUN_ID}/execute-c11-b`,
      {
        method: "POST",
  ...
  it("proceeds when the gate is enabled and the session cookie verifies", async () => {
    ...
    expect(response.status).toBe(200);
    expect(executeC11RealWebhookReplayMock).toHaveBeenCalledWith(VALID_RUN_ID);
  });
...
describe("POST /api/chaos/runs/[runId]/execute-c11-b — service result mapping", () => {
  beforeEach(() => {
    getAccessGateEnvMock.mockReturnValue({ mode: "disabled" });
  });

  it("maps COMPLETED to 200 with safe metadata only, replayAttemptCount fixed at 1", async () => {
    executeC11RealWebhookReplayMock.mockResolvedValue({
      kind: "COMPLETED",
      chaosRunId: VALID_RUN_ID,
      replayAttemptCount: 1,
  ...
  it("maps NOT_STARTABLE to 409 without leaking the reason category in the body", async () => {
    ...
    expect(text).not.toContain("SOURCE_EVIDENCE_UNRESOLVED");
  });
  ...
  it("maps FAILED to 500 without leaking the reason category, chaos run id internals, or any raw error", async () => {
    ...
    const text = await response.clone().text();
    expect(text).not.toContain("POST_STATE_VERIFICATION_FAILED");
  });

  it("maps a thrown error to a safe 500, never exposing the error message", async () => {
    executeC11RealWebhookReplayMock.mockRejectedValue(
...
describe("app/api/chaos/runs/[runId]/execute-c11-b/route.ts — module surface", () => {
  ...
  it("has no arbitrary target/authorization/replay-count field name in the FUNCTIONAL code (the module doc comment legitimately names these fields to document what the route refuses to accept, so only non-comment lines are checked here)", async () => {
    const fs = await import("node:fs");
    const path = await import("node:path");
    const source = fs.readFileSync(
      path.resolve(
  ...
  it("never imports the C01 replay service or route", async () => {
    const fs = await import("node:fs");
    const path = await import("node:path");
    const source = fs.readFileSync(
      path.resolve(
    ...
    expect(source).not.toMatch(/from\s*["']@\/lib\/chaos\/replay-service["']/);
    expect(source).not.toMatch(/executeC01Replay/);
  });
});
